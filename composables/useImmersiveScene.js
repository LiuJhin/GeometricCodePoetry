...
export default function useImmersiveScene() {
  ...
  const createNeonLines = () => {
    ...
    const createLightTrack = () => {
      ...
      trackLight.position.set(
        trackPoints[Math.floor(trackPoints.length / 2)].x,
  ...
  const onMouseMove = (event) => {
    ...
    if (particles && particles.material.uniforms) {
      particles.material.uniforms.mousePosition = {
    ...
    if (neonLines) {
      ...
      if (neonLines.userData.lightTracks) {
        neonLines.userData.lightTracks.forEach((track) => {
          ...
          // 获取轨道上的位置
          const position = lightPoint.userData.trackCurve.getPointAt(
            lightPoint.userData.trackTime
          );
      ...
      neonLines.children.forEach((line, i) => {
        ...
        for (let j = 0; j < originalPoints.length; j++) {
          ...
          const rippleWave =
            Math.sin(dist * 0.5 - elapsedTime * 3) * rippleStrength * 5;

          positions[idx] =
            originalPoint.x + waveX + (originalPoint.x / dist) * rippleWave;
          positions[idx + 1] =
            originalPoint.y + waveY + (originalPoint.y / dist) * rippleWave;
          positions[idx + 2] =
            originalPoint.z + waveZ + (originalPoint.z / dist) * rippleWave;
        }
